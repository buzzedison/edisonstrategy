...
import { ArrowUpRight, CheckCircle2, Compass, LineChart, Target, Users } from 'lucide-react';
import { getMarketingPageContent } from '@/lib/marketingPages';

export default async function ServicesStartup() {
  const content = await getMarketingPageContent('services-startup');
  const serviceSection = content.sections.find((section) => section.id === 'services');
  const winsSection = content.sections.find((section) => section.id === 'wins');
  const icons = [Target, Compass, LineChart, Users];
      ...
      <section className="pt-40 pb-24 px-6 lg:px-8 relative overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto relative z-10 grid lg:grid-cols-2 gap-14 items-center">
          ...
          <div className="bg-brand-stone/35 border border-brand-stone/60 p-4">
            <Image
              src={content.hero.imageUrl || '/image/edisonstart.png'}
              alt={content.hero.imageAlt || 'Edison supporting startup founders'}
              width={700}
              height={520}
      ...
      <section className="py-24 px-6 lg:px-8 bg-brand-stone/20 border-y border-brand-stone/50">
        <div className="max-w-7xl mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
            {(serviceSection?.cards || []).map((item, index) => {
