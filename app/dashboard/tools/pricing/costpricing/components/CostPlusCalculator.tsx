...
const CostPlusCalculator = () => {
  ...
  const signOut = async () => {
    ...
    if (error) {
      ...
      setStatusMessage("Error signing out. Please try again.");
    } else {
      setUser(null);
  ...
  return (
    <section className="bg-gradient-to-br from-gray-50 to-gray-100 py-16">
      <div className="container mx-auto px-4">
        <motion.h2 initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }} className="text-5xl font-bold mb-12 text-center text-gray-800">
        ...
        <div className="max-w-4xl mx-auto bg-white shadow-2xl rounded-3xl p-8">
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }} className="flex justify-between items-center mb-8">
            ...
            {user ? (
              <button
                onClick={signOut}
                className="px-4 py-2 bg-red-500 text-white rounded-full hover:bg-red-600 focus:outline-none focus:ring-2 focus:ring-red-500 transition-colors duration-300"
              >
                Sign Out
              </button>
            ) : (
          ...
          <motion.div ref={resultsRef} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }} className="bg-gradient-to-r from-blue-500 to-purple-600 p-8 rounded-3xl text-white">
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}>
            <button onClick={downloadPDF} className="px-6 py-3 bg-green-500 text-white rounded-full">
              Download Results as PDF
            </button>
          </motion.div>

          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }} className="mt-8">
            <button onClick={saveCalculation} className="px-6 py-3 bg-green-500 text-white rounded-full">
              Save Calculation
            </button>
          </motion.div>
